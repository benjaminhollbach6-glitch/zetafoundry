...
async def _audit_rates(request):
    ...
    def _parse_windows(qp, default="5,60,300"):
        ...
        for t in toks:
            ...
            if 1 <= w <= 3600 and w not in seen:
                ...
                if len(wins) >= 8:
                    break
        return wins or [5,60,300]

    def _decimals(qp):
        try:
            d = int(_first(qp, "dec", 2))
        except Exception:
            d = 2
        return 0 if d < 0 else 6 if d > 6 else d
    ...
    # flat=1 -> altes flaches Schema
    want_flat = str(_first(request.query_params, "flat", "0")).lower() in ("1","true","yes")
    if want_flat:
        flat = {}
        for w in wins:
            flat[f"count_{w}s"] = int(base.get(f"count_{w}s", 0))
            flat[f"rps_{w}s"]   = R(base.get(f"rps_{w}s", 0.0))
        flat["overall_count"] = int(base.get("overall_count", 0))
        flat["overall_rps"]   = R(base.get("overall_rps", 0.0))
        return JSONResponse(flat, headers={"Cache-Control":"no-store"})

    # default: strukturiertes Schema
    windows = [{"w": w, "count": int(base.get(f"count_{w}s", 0)), "rps": R(base.get(f"rps_{w}s", 0.0))} for w in wins]
    out = {
        "windows": windows,
        "overall": {
            "count": int(base.get("overall_count", 0)),
